...
const CLAIM_WINDOW_MS = 72 * 60 * 60 * 1000; // окно повторной претензии после завершения сделки — должно совпадать с CLAIM_WINDOW_MS в src/db.js
...
// Галерея скриншотов только для просмотра (без кнопки удаления) — переиспользует .shots-edit/.shot-thumb
function evidenceGalleryHtml(urls) {
  if (!urls || !urls.length) return '';
  return `<div class="shots-edit">${urls.map((u) => `<a class="shot-thumb" href="${esc(u)}" target="_blank" rel="noopener"><img src="${esc(u)}" alt=""></a>`).join('')}</div>`;
}

// Общая логика загрузки скриншотов-доказательств (передача товара / спор / ответ на спор) —
// та же схема, что и для скриншотов статистики канала при создании товара.
function wireEvidenceUpload(root, boxId, fileId, evidence, max = 8) {
  const box = root.querySelector('#' + boxId);
  const fileInput = root.querySelector('#' + fileId);
  const render = () => {
    box.innerHTML = evidence.map((u, i) =>
      `<div class="shot-thumb"><img src="${esc(u)}"><button type="button" class="shot-rm" data-rm="${i}">${ic('x-lg')}</button></div>`).join('');
    box.querySelectorAll('[data-rm]').forEach((b) =>
      b.addEventListener('click', () => { evidence.splice(Number(b.dataset.rm), 1); render(); }));
  };
  render();
  fileInput.addEventListener('change', async () => {
    const files = Array.from(fileInput.files || []);
    fileInput.value = '';
    for (const file of files) {
      if (evidence.length >= max) { toast(`Максимум ${max} скриншотов`); break; }
      try {
        const { url } = await API.post('/upload', { image: await compressImage(file) });
        evidence.push(url); render();
      } catch (e) { toast('Не удалось загрузить изображение'); }
    }
  });
}
function evidenceUploadFieldHtml(label, boxId, fileId) {
  return `<div class="field"><label>${esc(label)}</label>
    <div class="shots-edit" id="${boxId}"></div>
    <label class="upload-btn" for="${fileId}">${ic('image')} Добавить скриншот (необязательно)</label>
    <input id="${fileId}" type="file" accept="image/*" multiple hidden></div>`;
}

// Блоки с уже сохранёнными доказательствами передачи/спора — показываются в деталях сделки
function dealEvidenceBlocksHtml(d) {
  let html = '';
  if (d.delivery_proof) {
    html += `<div class="section-label mt12">Как продавец передал товар</div>
      <div class="list-group"><div class="ios-row" style="display:block;padding:12px 14px"><p style="margin:0 0 8px;white-space:pre-wrap">${esc(d.delivery_proof)}</p>${evidenceGalleryHtml(d.delivery_proof_evidence)}</div></div>`;
  }
  if (d.dispute_reason) {
    ...
    html += `<div class="section-label mt12">Причина спора${opener ? ` (открыл: ${opener})` : ''}</div>
      <div class="list-group"><div class="ios-row" style="display:block;padding:12px 14px"><p style="margin:0 0 8px;white-space:pre-wrap">${esc(d.dispute_reason)}</p>${evidenceGalleryHtml(d.dispute_evidence)}</div></div>`;
  }
  if (d.dispute_response) {
    html += `<div class="section-label mt12">Ответ на спор</div>
      <div class="list-group"><div class="ios-row" style="display:block;padding:12px 14px"><p style="margin:0 0 8px;white-space:pre-wrap">${esc(d.dispute_response)}</p>${evidenceGalleryHtml(d.dispute_response_evidence)}</div></div>`;
  }
  return html;
}
...
function renderDealActions(d, role) {
  ...
  // Окно повторной претензии: 72 ч после завершения сделки (см. CLAIM_WINDOW_MS в src/db.js)
  const claimMsLeft = d.status === 'completed' && d.updated_at ? Number(d.updated_at) + CLAIM_WINDOW_MS - Date.now() : 0;
  if (claimMsLeft > 0) btns.push(['secondary', 'dispute', 'exclamation-triangle', `Открыть спор (осталось ${Math.max(1, Math.ceil(claimMsLeft / 3600000))} ч)`]);
  if (d.status === 'disputed' && d.disputed_by && state.me && Number(d.disputed_by) !== Number(state.me.id) && !d.dispute_response) {
    btns.push(['secondary', 'respond', 'reply', 'Ответить на спор']);
  }
  ...
  if (d.status === 'completed') html = `<p class="text-hint" style="text-align:center;padding:8px">Сделка успешно завершена${claimMsLeft > 0 ? '. Если что-то не так — можно открыть спор в течение 72 часов после завершения.' : ''}</p>` + html;
  if (d.status === 'cancelled') html = `<p class="text-hint" style="text-align:center;padding:8px">Сделка отменена, средства возвращены покупателю</p>`;
  if (d.status === 'disputed') html = `<p class="text-hint" style="text-align:center;padding:8px">${ic('exclamation-triangle')} Спор рассматривается администратором</p>` + html;
  box.innerHTML = html;
...
async function dealAction(d, action, role) {
  ...
  if (action === 'deliver') return deliverDealWithProof(d);
  if (action === 'dispute') return disputeDealWithEvidence(d);
  if (action === 'respond') return respondToDisputeForm(d);
  const texts = {
...
// Передача товара на проверку — обязательное текстовое описание + необязательные скриншоты
function deliverDealWithProof(d) {
  const box = document.getElementById('deal-actions');
  const evidence = [];
  box.innerHTML = `
    <div class="escrow-note mt8">${ic('info-circle')} Опишите, что именно и как вы передали (ссылку, логин/доступ и т.д.) — это поможет при споре.</div>
    <div class="field"><textarea id="dp-proof" maxlength="2000" placeholder="Например: передал права администратора канала аккаунту @username, отправил ссылку-приглашение..."></textarea></div>
    ${evidenceUploadFieldHtml('Доказательства (скриншоты)', 'dp-shots', 'dp-file')}
    <button class="btn success mt8" id="dp-submit">${ic('box-seam')} Передать на проверку</button>`;
  wireEvidenceUpload(box, 'dp-shots', 'dp-file', evidence);
  document.getElementById('dp-submit').addEventListener('click', async () => {
    const proof = document.getElementById('dp-proof').value.trim();
    if (proof.length < 10) return toast('Опишите подробнее, что вы передали (не менее 10 символов)');
    try {
      await API.post(`/deals/${d.id}/deliver`, { proof, evidence });
      haptic('success'); toast('Передано на проверку покупателю'); closeSheet(); loadDealsList();
    } catch (e) { toast(e.message); haptic('error'); }
  });
}

// Открытие спора — обязательная причина + необязательные скриншоты; работает и для «Завершена»
// в течение окна претензий (72ч) — сервер сам проверит окно и вернёт понятную ошибку, если оно истекло.
function disputeDealWithEvidence(d) {
  const box = document.getElementById('deal-actions');
  const evidence = [];
  const isReopen = d.status === 'completed';
  box.innerHTML = `
    <div class="escrow-note mt8">${ic('exclamation-triangle')} ${isReopen
      ? 'Спор по уже завершённой сделке рассмотрит администратор. Если доводы подтвердятся, деньги могут списать с продавца обратно вам.'
      : 'Опишите проблему подробно — спор рассмотрит администратор.'}</div>
    <div class="field"><textarea id="ds-reason" maxlength="2000" placeholder="Опишите, что пошло не так..."></textarea></div>
    ${evidenceUploadFieldHtml('Доказательства (скриншоты)', 'ds-shots', 'ds-file')}
    <button class="btn danger mt8" id="ds-submit">${ic('exclamation-triangle')} Открыть спор</button>`;
  wireEvidenceUpload(box, 'ds-shots', 'ds-file', evidence);
  document.getElementById('ds-submit').addEventListener('click', async () => {
    const reason = document.getElementById('ds-reason').value.trim();
    if (reason.length < 10) return toast('Опишите проблему подробнее (не менее 10 символов)');
    try {
      await API.post(`/deals/${d.id}/dispute`, { reason, evidence });
      haptic('success'); toast('Спор открыт, администратор скоро разберётся'); closeSheet(); loadDealsList();
    } catch (e) { toast(e.message); haptic('error'); }
  });
}

// Ответ второй (не открывавшей спор) стороны — своя версия + доказательства для администратора
function respondToDisputeForm(d) {
  const box = document.getElementById('deal-actions');
  const evidence = [];
  box.innerHTML = `
    <div class="escrow-note mt8">${ic('info-circle')} Изложите свою версию произошедшего — администратор учтёт обе стороны при решении.</div>
    <div class="field"><textarea id="dr-response" maxlength="2000" placeholder="Ваш ответ на спор..."></textarea></div>
    ${evidenceUploadFieldHtml('Доказательства (скриншоты)', 'dr-shots', 'dr-file')}
    <button class="btn mt8" id="dr-submit">${ic('reply')} Отправить ответ</button>`;
  wireEvidenceUpload(box, 'dr-shots', 'dr-file', evidence);
  document.getElementById('dr-submit').addEventListener('click', async () => {
    const response = document.getElementById('dr-response').value.trim();
    if (response.length < 10) return toast('Опишите свою версию подробнее (не менее 10 символов)');
    try {
      await API.post(`/deals/${d.id}/dispute/respond`, { response, evidence });
      haptic('success'); toast('Ответ отправлен администратору'); closeSheet(); loadDealsList();
    } catch (e) { toast(e.message); haptic('error'); }
  });
}
